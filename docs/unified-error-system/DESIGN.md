...
The Unified Error System provides a compile-time error definition framework built on two standards: GTS (Global Type System) for error classification and RFC 9457 (Problem Details) for wire format. Every error is defined as a Rust struct annotated with `#[struct_to_gts_schema]` (from the `gts-macros` crate) and implements the `GtsError` trait (from `modkit-errors`), which together generate schema constants and provide RFC 9457 conversion via `into_problem()`.

The architecture enforces a security-first approach: error responses contain only sanitized, structured data (`type`, `title`, `status`, `trace_id`, `metadata`), while full error details (chains, stack traces, internal state) are logged server-side and correlated via trace ID. The `detail`, `instance`, `code`, and `errors` fields from RFC 9457 are intentionally omitted from the Problem struct to prevent sensitive data leakage and keep the response surface minimal.
...
graph TD
    subgraph Compile Time
        Dev[Module Developer] -->|annotates structs| Macro["#[struct_to_gts_schema] + GtsError trait"]
        Macro -->|generates| Constants[gts_schema_id / gts_type_uri / into_problem]
    end
    ...
    subgraph Runtime
        ...
        ModkitErrors -->|Span::current| Tracing[tracing crate]
...
|-------------|-----------------|
| `cpt-cf-ues-fr-gts-type-id` | 2-segment GTS chain model with `gts://` URI prefix; `gts_schema_id()` method generated by `#[struct_to_gts_schema]` macro |
| `cpt-cf-ues-fr-trace-id` | Trace-id enrichment inlined in `Problem::IntoResponse` and `finalize()` helper; W3C trace-id (32 hex) extracted from current tracing span; `Option<String>` field on Problem |
| `cpt-cf-ues-fr-error-registration` | Error metadata collected per error struct; modules register via `on_ready` lifecycle hook |
| `cpt-cf-ues-fr-rfc9457-schema` | `Problem` struct with RFC 9457 fields; serializes to `application/problem+json` |
| `cpt-cf-ues-fr-compile-time-def` | `#[struct_to_gts_schema]` macro generates `GtsSchema` impl; `GtsError` trait provides `into_problem()` |
| `cpt-cf-ues-fr-metadata-fields` | Struct fields serialize as nested `metadata` JSON object via serde; no builder API |
| `cpt-cf-ues-fr-system-errors` | Pre-defined error structs in `modkit-errors` for common HTTP error categories (bad_request, forbidden, not_found, conflict, unsupported_media_type, validation_failed, internal, config, unknown) |
| `cpt-cf-ues-fr-machine-readable` | GTS `type` field provides deterministic, parseable error codes for programmatic handling |
| `cpt-cf-ues-fr-ergonomic-api` | `GtsError` trait provides `STATUS`, `TITLE` constants, `gts_type_uri()` method, and `into_problem()` conversion |

#### NFR Allocation

| NFR ID | NFR Summary | Allocated To | Design Response | Verification Approach |
|--------|-------------|--------------|-----------------|----------------------|
| `cpt-cf-ues-nfr-no-sensitive-data` | No sensitive data in responses | `GtsError` trait + `#[serde(skip_serializing)]` attribute | Metadata populated only from declared struct fields; `#[serde(skip_serializing)]` excludes internal fields from response; no `detail` field | Security scan of all error responses; dylint lint for raw Problem construction |
...
|-------|---------------|------------|
| Schema Macro | GTS schema generation, schema ID constants | `#[struct_to_gts_schema]` proc macro from `gts-macros` crate |
| Error Trait | Error interface, RFC 9457 serialization | `GtsError` trait in `modkit-errors` |
| Types | Problem struct, BaseErrorV1 | Rust structs in `modkit-errors` |
| Runtime | Trace ID extraction, serialization, HTTP response integration | tracing, serde, http crate |
...
Error responses expose only `type`, `title`, `status`, `trace_id`, and sanitized `metadata`. The RFC 9457 `detail`, `instance`, `code`, and `errors` fields are removed from the Problem struct to prevent sensitive data leakage. Full error details are logged server-side with trace_id for correlation.
...
| Dynamic metadata injection of sensitive data | CWE-209 | Runtime `.with_metadata()` could inject credentials, PII, or internal state | No builder API — metadata populated exclusively from declared struct fields | `cpt-cf-ues-principle-metadata-from-fields`, `#[serde(skip_serializing)]` attribute |
...
| Bypass via raw Problem construction | CWE-209 | Developers construct Problem struct directly, bypassing trait safeguards | Enforced via dylint lint rules and code review checklist; all errors must go through `into_problem()` | NFR `cpt-cf-ues-nfr-no-sensitive-data` verification approach |

### 2.5 Capacity & Cost Budgets

Not applicable — `modkit-errors` is a compile-time code generation library with minimal runtime overhead. Error conversion via `into_problem()` (see [modkit-errors](../../libs/modkit-errors)) is a synchronous in-memory operation with negligible latency. Heap allocations are limited to the created `Problem` struct and its fields: depending on error content, this includes `String` allocations for `type` and `title`, a `StatusCode` for `status`, an `Option<String>` for `trace_id`, plus an `Option<HashMap<String, Value>>` for metadata entries. No I/O, async operations, or external resource allocations occur during conversion.
...
|--------|-------------|--------|
| Problem | RFC 9457 error response with GTS type, title, status, trace_id, metadata. Fields `detail`, `instance`, `code`, `errors` are intentionally omitted. | [problem.rs](../../libs/modkit-errors/src/problem.rs) |
| BaseErrorV1\<M\> | Root GTS schema anchor (`gts.cf.core.errors.err.v1~`); public generic struct where `M` is the metadata type (defaults to `()`) | [gts_error.rs](../../libs/modkit-errors/src/gts_error.rs) |

**Relationships**:
- Metadata struct → BaseErrorV1: All errors are `BaseErrorV1<MetadataStruct>` forming the 2-segment GTS chain
- BaseErrorV1\<M\> → Problem: `into_problem()` (via `GtsError` trait) converts to RFC 9457 Problem struct
- Problem → Trace context: `trace_id` populated from current tracing span via `Problem::IntoResponse` (axum) or `finalize()` helper

### 3.2 Component Model

```
modkit-errors (crate)
├── GtsError trait (error interface)
│   ├── Defines: STATUS (u16), TITLE (&str) constants
│   ├── Provides: gts_type_uri() → &str (cached gts:// URI)
│   └── Provides: into_problem() → Problem (RFC 9457 struct)
├── BaseErrorV1<M> (root schema anchor)
│   ├── #[struct_to_gts_schema]: schema_id = "gts.cf.core.errors.err.v1~", base = true
│   ├── gts_type: GtsSchemaId (#[serde(skip_serializing)])
│   └── metadata: M (#[serde(skip_serializing_if = "is_empty_metadata")])
├── is_empty_metadata() (conditional serialization helper)
├── Problem struct
│   ├── type: String (GTS URI)
│   ├── title: String (static)
...
├── finalize(problem, trace_id) (trace-id enrichment helper)
├── Field attributes (standard serde)
│   ├── (default): field → metadata entry
│   ├── #[serde(skip_serializing)]: exclude from response
│   └── Standard serde attributes for serialization control
└── System error catalog (cf.core.errors.*)
    ├── bad_request, forbidden, not_found, conflict (4xx)
    ├── unsupported_media_type, validation_failed (4xx)
    ├── internal, config, unknown (5xx)
```
...
**Responsibility**: Provides the `GtsError` trait and `BaseErrorV1<M>` struct (`cpt-cf-ues-interface-gts-error-macro`), `Problem` struct (`cpt-cf-ues-interface-problem`), `finalize()` helper for trace-id enrichment (`cpt-cf-ues-interface-trace-id-fn`), `Problem::IntoResponse` axum integration (auto-enriches `trace_id`, sets `X-Trace-Id` and `X-Error-Code` headers), serde-based field attributes, and the system error catalog (`cf.core.errors.*`). Single crate that all modules depend on for error handling. Uses `gts-macros` crate for `#[struct_to_gts_schema]` proc macro.
...
| `X-Trace-Id` | When `trace_id` is present | Trace ID (32 hex chars) |
| `X-Error-Code` | When `type` is a valid header value | GTS error type URI |
| `Retry-After` | 429, 503 | Seconds to wait (set by middleware, not Problem) |
...
|------------|----------------|---------|
| `tracing` | `Span::current().id()` | Extract span ID for trace_id enrichment in `Problem::IntoResponse` (axum feature) |
...
sequenceDiagram
    ...
    Handler ->> ErrMap: From<DomainError> for Problem
    ...
    ErrMap -->> Handler: Problem { type, title, status, metadata }
    Handler ->> Handler: Problem.into_response()
    Note over Handler: into_response() internally:
    Handler ->> Tracing: tracing::Span::current().id()
    alt Trace context available
        Tracing -->> Handler: Some(span_id) → format!("{:032x}", u64)
        Handler ->> Handler: problem.with_trace_id(trace_id_32hex)
    else No active span
        Tracing -->> Handler: None (trace_id omitted from response)
    end
    Note over Handler: Sets headers: Content-Type, X-Trace-Id, X-Error-Code
    Handler -->> Client: HTTP {status} + application/problem+json
```

**Description**: When a domain operation fails, the `From<DomainError> for Problem` implementation matches the variant, constructs the appropriate error struct, and calls `into_problem()` (via the `GtsError` trait) which serializes metadata from struct fields into a `Problem`. The `Problem::into_response()` axum integration then enriches the Problem with a W3C trace-id from the current tracing span (if available) and returns it as an `application/problem+json` response with appropriate headers.
...
**Description**: During the `on_ready` lifecycle phase, each module collects its error definitions and registers them with the types registry. This enables runtime error schema discovery and validation.
...
**Platform errors** (`cf.core.errors.*`):
- `cf.core.errors.bad_request.v1` — 400 Bad Request
- `cf.core.errors.forbidden.v1` — 403 Forbidden
- `cf.core.errors.not_found.v1` — 404 Not Found
- `cf.core.errors.conflict.v1` — 409 Conflict
- `cf.core.errors.unsupported_media_type.v1` — 415 Unsupported Media Type
- `cf.core.errors.validation_failed.v1` — 422 Validation Failed
- `cf.core.errors.internal.v1` — 500 Internal Server Error
- `cf.core.errors.config.v1` — 500 Configuration Error
- `cf.core.errors.unknown.v1` — 500 Unknown Error

**Module errors** follow the pattern `cf.<module_name>.errors.<type>.v1` and are defined within each module's crate.

### Field Attribute Reference

| Attribute | Effect |
|-----------|--------|
| _(none)_ | Field included in serialized `metadata` object |
| `#[serde(skip_serializing)]` | Field excluded from response (logged server-side only) |
| `#[serde(skip_serializing_if = "...")]` | Field conditionally excluded (e.g., `is_empty_metadata` omits empty metadata) |
| Standard serde attributes | Full serde attribute support for serialization control (rename, flatten, etc.) |
